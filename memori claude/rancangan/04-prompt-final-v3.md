...
> **Versi 3 — FINAL.** Menggantikan v1 dan v2 sepenuhnya. Jangan pakai berkas prompt lain lagi;
> berkas ini sudah memuat seluruh koreksinya di tempatnya masing-masing.
...
>
> **Sebelum mulai:** simpan dulu kondisi kode sekarang (`git commit`, lalu kerjakan di cabang baru).
> Fitur ini menyentuh model data, form, 10 template, 4 eksportir, dan mesin skor sekaligus.
>
...
> Perubahan v3 atas v2: rumus skor dibuat deterministik (§9.2), jalur jenjang pemula ditambahkan
> (§9.5), penggantian nama "Skor ATS" dipersempit (§3.7), `gabungKePengalaman` menempel pada entri
> pengalaman tertentu (§3.8), dan kewajiban PDP untuk data verifikator (§5).
...
7. ⭐ **BERHENTI di akhir tiap fase.** Jangan mulai fase berikutnya sebelum saya bilang lanjut.
   Di akhir tiap fase: jalankan aplikasinya, tunjukkan buktinya, sebutkan berkas yang berubah,
   lalu tunggu. Fase 5 (skor) **tidak boleh** dikerjakan di sesi yang sama dengan fase 3–4
   (render) — kalau struktur dan angka berubah bersamaan, tidak ada yang tahu mana penyebab
   skor bergeser.
8. **Jangan mengarang rumus, ambang, atau angka.** Kalau prompt ini tidak menyebut angkanya,
   tanyakan — jangan diisi sendiri. Ini berlaku terutama untuk §9 (penilaian).

---

## 1. Kondisi aplikasi saat ini — SUDAH DIPERIKSA LANGSUNG DI KODE

Bagian ini bukan asumsi. Isinya hasil pembacaan repo pada 4 September 2026, commit `a2bc0be`.
**Tiga di antaranya membatalkan premis yang beredar di draf sebelumnya — baca sampai habis
sebelum menulis kode.**

**Yang terkonfirmasi benar:**

- Next.js 16 + Prisma 7 + NextAuth. Berkas kunci: `src/lib/resume/types.ts`,
  `src/lib/resume/templates.ts`, `src/lib/ats/engine.ts` (839 baris), `src/lib/ats/types.ts`,
  `src/components/editor/sections.tsx`, `src/components/preview/ResumeDocument.tsx`,
  `src/lib/docx/build.ts`, `src/lib/resume/download.ts`.
- **10 template**, semuanya satu kolom. `TemplateId` = CLASSIC · MODERN · COMPACT · EXECUTIVE ·
  MINIMAL · TIMELINE · ACADEMIC · GOVERNMENT · PORTRAIT · PROFILE.
- **5 dimensi skor.** `DimensionKey` = `completeness` · `parseability` · `contentQuality` ·
  `keywordMatch` · `structure`. Tiap `AtsFinding` sudah punya field `section` untuk melompat
  ke field bermasalah — pakai mekanisme itu, jangan bikin yang baru.

### ⚠️ KOREKSI 1 — bagian `project` SUDAH ADA. Ini bukan bagian ke-12.

`SectionKey` di `src/lib/resume/types.ts:45` berisi **11 nilai**:
`summary · experience · education · skill · project · certification · organization · award ·
language · publication · custom`.

`project` sudah ada, dan bentuknya nyaris sama dengan "field umum" yang dirancang di §5:

```ts
export interface ProjectItem {          // types.ts:136
  id: string; name: string; role: string; url: string;
  startDate: string; endDate: string; bullets: string[];
}
```

**Jadi tugasnya adalah MEMPERLUAS `project` yang sudah ada, bukan menambah bagian baru bernama
PORTOFOLIO di sebelahnya.** Membuat bagian kedua yang tumpang tindih akan membingungkan pengguna
dan memecah data yang seharusnya satu.

Pemetaan yang wajib dipakai: `name` → `judul` · `role` → `peran` · `url` → `tautan[0]` ·
`startDate`/`endDate` → `mulai`/`selesai` · `bullets` → `poin`.
Yang **ditambahkan**: `konteks` (wajib), `ringkasan`, `inti`, `detailTambahan`, `verifikator`,
`refleksi`, `parentPengalamanId`, `kataKunci`, `polaOverride`.
`SectionKey` tidak bertambah; judul bagiannya yang berubah mengikuti pola (§6).

### ⚠️ KOREKSI 2 — bagian `publication` juga sudah ada

```ts
export interface PublicationItem {      // types.ts:181
  id: string; title: string; publisher: string; date: string; url: string; doi: string;
}
```

Itu **persis pola `karya-terkredit`** dalam bentuk sederhana. Jangan membuat pola ini sebagai
struktur terpisah yang menduplikasi `publications`. Perluas `PublicationItem` dengan
`tipeLuaran`, `peranSaya`, dan `indeksasiTier`; `title`+`publisher`+`date` menjadi bahan
`sitasiLengkap`, `url`/`doi` menjadi `pengenalPersisten`.

Konsekuensi: pengguna berpola `karya-terkredit` memakai bagian `publication` yang diperluas,
**bukan** bagian `project`. Pola menentukan bagian mana yang aktif dan bagaimana ia dirender.

### ⚠️ KOREKSI 3 — penalti panjang halaman itu NYATA, dan keras

Di `src/lib/ats/engine.ts:617`, di dalam `scoreStructure`:

```ts
const lengthRatio = pages === 1 ? 1 : pages === 2 ? 0.75 : 0.25;
```

CV 2 halaman kehilangan 25% dimensi `structure`; 3 halaman ke atas kehilangan 75%.
Ada pula `AtsFinding` bernada peringatan (`m.lengthTooLong`) untuk `pages > 2`.

Jadi instruksi §3.5 ("hapus penalti panjang halaman untuk SEMUA pola") **bukan tindakan kosong** —
ia mengubah `scoreStructure` secara nyata. Ganti `lengthRatio` menjadi konstanta `1`, ubah temuan
`lengthTooLong` jadi bernada **netral dan informatif** (severity `info`, teks "CV Anda ±2 halaman —
lazim untuk tingkat menengah ke atas"), dan sesuaikan teksnya di `src/lib/ats/messages.ts` serta
`document-messages.ts`. Laporkan pergeseran skor yang ditimbulkannya pada CV contoh.

### Yang belum saya verifikasi — periksa sendiri di Fase 1

Simpan-otomatis <1 detik, unggah/bandingkan 5 CV di peramban, ekspor/impor JSON, hapus akun,
ukuran kertas A4/Letter/Legal/F4, dan pencocokan iklan lowongan. Semuanya disebut di beranda;
konfirmasikan letak kodenya sebelum menyentuhnya, dan laporkan bila ada yang berbeda.
...
⚠️ **Batas penggantian nama — baca dengan teliti.** Yang diganti adalah **nama angkanya**, bukan
kata "ATS" di seluruh produk:

- **Ganti:** setiap tempat yang menyebut hasil penilaian sebagai "Skor ATS" → dua angka di atas.
- **Pertahankan:** kata "ATS" sebagai istilah kategori — nama produk, judul beranda, penjelasan
  masalah di beranda. "CV ATS" adalah kata yang diketik pengguna Indonesia di mesin pencari dan
  nama domain produk ini sendiri; menghapusnya membuang penemuan produk tanpa menambah kejujuran.
- **Wajib ditambahkan:** tooltip permanen di sebelah kedua angka —
  *"Angka ini menilai struktur dan kecocokan kata kunci CV Anda. Ia tidak memprediksi keputusan
  sistem perekrutan mana pun — tiap perusahaan menyetel filternya sendiri."*
...
2. **Sakelar `gabungKePengalaman`** — bila aktif, item dirender **di dalam entri pengalaman kerja
   tertentu**, bukan sekadar diletakkan di bawah judulnya. RChilli mengharapkan proyek bersarang
   pada satu blok pengalaman, dengan tanggal proyek **berada di dalam rentang masa kerja induknya**.
   Merender item lepas di bawah judul `PENGALAMAN KERJA` belum memenuhi itu, dan menciptakan
   risiko baru: pemberi kerja yang sama tercetak dua kali.

   Yang wajib dibangun:
   - Field `parentPengalamanId?: string` pada `ItemPortofolio`.
   - Saat sakelar diaktifkan, untuk tiap item tawarkan daftar entri pengalaman kerja sebagai induk.
     Cocokkan otomatis bila `konteks` sama persis dengan nama perusahaan pada entri itu.
   - **Validasi tanggal:** `mulai`/`selesai` item harus berada di dalam rentang entri induknya.
     Bila di luar, beri peringatan dan tawarkan lepaskan dari induk.
   ...
   - Item tanpa induk (freelance, tugas kuliah, open source) tetap berada di bagian portofolio
     sendiri meskipun sakelar aktif.
...
type ProfilPortofolio = {
  ...
  // ⭐ Menentukan field mana yang WAJIB dan bagaimana skor dinormalisasi (§9.5).
  // Tanpa ini, mahasiswa dinilai dengan ambang senior dan pasti mendapat skor rendah.
  jenjang: 'mahasiswa' | 'baru-lulus' | '1-3-tahun' | '4-8-tahun' | 'di-atas-8-tahun'
}
...
type ItemPortofolio = {
  ...
  parentPengalamanId?: string    // ⭐ induk saat gabungKePengalaman aktif (§3.8)
  ...
  // Didefinisikan HANYA di sini (tingkat item). Kemunculannya dikendalikan
  // `butuhVerifikator` pada skema pola — jangan mendefinisikannya lagi sebagai field inti.
  //
  // ⚠️ KEWAJIBAN PDP: ini data pribadi milik ORANG LAIN yang tidak menyetujui penyimpanannya,
  // dan aplikasi ini menyimpannya ke basis data — UU 27/2022 berlaku, bukan sekadar etika.
  // Wajib: (a) teks di bawah field — "Data ini tidak dicetak di CV dan tidak dikirim ke mana pun.
  // Simpan hanya bila Anda sudah izin ke orang tersebut."; (b) ikut terhapus saat pengguna
  // menghapus akun; (c) dikecualikan dari fitur ekspor/salin ke pihak mana pun.
...
**Field inti (6):** — `verifikator` **bukan** field inti; ia field tingkat item (§5), dinyalakan
oleh `butuhVerifikator: true` pada pola ini.
...
⚠️ **Ambang di tabel ini WAJIB disimpan sebagai data, bukan konstanta di dalam kode.**
Taruh di `lib/portfolio/ambang-profesi.ts`, tiap entri punya field `sumber` (nama peraturan) dan
`diperbarui` (tanggal) yang **ditampilkan di UI**. Begitu aplikasi menampilkan "180 dari 250",
pengguna akan memakainya untuk memutuskan perpanjangan izin praktik — kalau angkanya usang,
kerugiannya nyata.

**Sanggahan wajib di bawah progress bar (tidak bisa ditutup):**
...
**Dilarang** menulis kalimat yang menyatakan pengguna "sudah memenuhi syarat perpanjangan SIP".
...
|---|---|---|
| **STR** (Surat Tanda Registrasi) tenaga medis & kesehatan | Konsil Kesehatan Indonesia | ⭐ **SEUMUR HIDUP** untuk STR Definitif (UU 17/2023, aturan pelaksana PP 28/2024 — terverifikasi). Yang tetap berbatas waktu: STR Internsip, Pendidikan, Adaptasi, Penambahan Kompetensi, Sementara, Bersyarat |
...
**Rumusnya harus DETERMINISTIK — pakai rubrik di bawah ini persis, jangan mengarang sendiri.**
FAIP dinilai manusia yang membaca narasi; aplikasi ini harus menghasilkan angka yang sama untuk
data yang sama, dan tiap angka harus bisa diklik ke field penyebabnya.

**Q — peranan.** Nilai 0–3, dibaca dari `peran`, `kontribusi`, `tahapKeterlibatan`:
...
| 2 | peran spesifik **dan** minimal 1 kata kerja orang pertama pada `poin`/`inti` |
...
**R — tingkat kesulitan.** Nilai = jumlah kondisi yang terpenuhi, maksimal 3:
(a) skala terisi dengan angka **dan** satuan (`skalaProyek` / `volume` / `skalaDikelola`);
(b) `standarKode` / `metodeStandar` memuat ≥1 entri;
(c) `hasilTerukur` / `hasil` / `luaran` / `metrikDampak` memuat angka.

**Skor item** = `(Q × R) / 9 × 100`, lalu disesuaikan:

| Penyesuaian | Nilai | Syarat |
|---|---|---|
| Verifikator lengkap | **+8** | hanya untuk pola yang `butuhVerifikator` |
| Refleksi terisi ≥80 karakter | **+4** | maksimal +4, sekali per item, tidak akumulatif — tidak boleh ada jalur lain yang menambah skor dari field ini |
| Tidak ada tautan valid | **−15** | hanya untuk `karya-visual` dan `karya-terkredit`; pada pola lain tautan tidak memengaruhi skor |

Hasil akhir dijepit ke 0–100.

**P — banyaknya pengalaman** dipakai sebagai **pengali agregat**, bukan komponen per item:
...
P      = n < a           -> n / a       // kurang dari ideal, dipotong proporsional
         ...
         n > b           -> 1           // TIDAK dihukum; hanya b item terkuat yang dirender
```

⚠️ `b === null` harus ditangani eksplisit — tanpa itu aritmatika P meledak pada pola
`karya-terkredit`. Buat uji unitnya.

**Skor bagian** = `rata-rata(skor b item terbaik, atau semua item bila b null) × P`.

> ⚠️ **Jangan memakai ambang FAIP 600 / 3.000 / 6.000 sebagai skala.** Angka itu untuk akumulasi
> karier 3/8/16 tahun, bukan untuk satu bagian CV — meminjam strukturnya (P×Q×R) benar, meminjam
> angkanya salah kategori. Penyesuaian senioritas masuk lewat §9.5, bukan lewat ambang ini.

**Uji yang harus lolos:** 3 item lengkap + verifikator → Q=3, R=3 → 100, +8 → dijepit 100, P=1
→ **≥85 ✅**. 3 item tanpa angka dan tanpa tautan → Q≤1, R≤1 → ≤11 → **<50 ✅**.
Kriteria penerimaan §14 poin 4 baru bisa lolos setelah rubrik ini dipakai.
...
- **Bonus refleksi** (dari GMC revalidation & rubrik UKMPPG): field `refleksi` opsional —
  apa yang dipelajari, apa yang akan dilakukan berbeda. Murah diminta, sulit dipalsukan.
  **Dibatasi +4, sekali per item** (lihat tabel penyesuaian di atas).
- **Higiene tautan** dicek sebagai **validasi** (URL valid, bukan pemendek, maks 2 per item).
  Pengaruhnya ke skor hanya lewat penalti −15 pada dua pola yang mensyaratkan tautan — bukan
  sebagai komponen bernilai 10 seperti di v1.
- **`detailTambahan` yang tidak dicetak di CV** tetap ikut dihitung di **Kecocokan Lowongan**
  (kata kuncinya tetap milik pengguna), tapi **tidak** ikut faktor R.
...
### 9.4 Renormalisasi bobot — aturan eksplisit

v2 menyuruh "turunkan bobot dimensi lain secara proporsional" **dan** "skor CV lama tidak boleh
berubah sebelum portofolio diaktifkan". Dua kalimat itu hanya konsisten kalau aturannya ditulis:

```
portofolio.aktif === false  ->  bobot Bukti Karya = 0
                                5 dimensi lama memakai bobot aslinya, total 100

portofolio.aktif === true   ->  bobot Bukti Karya = bobotBuktiKarya pola
                                5 dimensi lama dikalikan (100 - bobotBuktiKarya) / 100
```

Saat pengguna mengaktifkan portofolio pertama kali, tampilkan satu baris penjelasan:
*"Bobot penilaian berubah karena bagian portofolio aktif. Skor sebelumnya: 87."*

### 9.5 ⭐ Penyesuaian jenjang — `wajib` tidak boleh sama untuk mahasiswa dan senior

Ini **menimpa** daftar `wajib` di §6 untuk pengguna jenjang pemula. Tanpa ini, mahasiswa dan
fresh graduate — sebagian besar pengguna aplikasi ini — menghadapi form yang mensyaratkan
`verifikator`, `hasilTerukur` berangka, dan `standarKode` yang memang belum mereka punya, lalu
mendapat skor rendah tanpa jalan keluar. Itu yang membuat orang berhenti memakai aplikasi.

| `jenjang` | Penyesuaian |
|---|---|
| `mahasiswa`, `baru-lulus` | `verifikator` **tidak wajib** — turun jadi saran, dengan teks *"dosen pembimbing atau ketua tim juga sah"*. `hasilTerukur` boleh berupa hasil pengujian/tugas, bukan hasil bisnis. `statusKarya` bernilai "tugas kuliah" / "latihan pribadi" **tidak menurunkan skor** — hanya wajib diberi label jujur. Batas bawah `rentangItemIdeal` turun jadi **2**. |
| `1-3-tahun` | default penuh; `verifikator` disarankan, belum wajib |
| `4-8-tahun`, `di-atas-8-tahun` | default penuh; `verifikator` **wajib** untuk `proyek-teknis` |

Nada saran perbaikan untuk jenjang pemula juga berbeda — jangan memakai kalimat yang mengandaikan
pengalaman kerja: *"Tugas kuliah tetap dihitung. Yang membedakannya: sebut hasil pengukurannya
dan apa yang Anda kerjakan sendiri."*
...
1. **Onboarding singkat — tiga pertanyaan pendek.**
   ...
   - ⭐ *"Sejauh mana pengalaman Anda?"* — satu baris, 5 pilihan: **mahasiswa · baru lulus ·
     1–3 tahun · 4–8 tahun · di atas 8 tahun.** Mengisi `jenjang` dan menentukan field mana yang
     wajib (§9.5). **Jangan dilewati** — ini yang mencegah mahasiswa dinilai dengan ambang senior.
...
8. **Sakelar "Gabung ke Pengalaman Kerja"** (§3.8) — di level bagian, dengan penjelasan jujur.
   ⚠️ **Jangan aktif otomatis.** Mengubah struktur CV pengguna tanpa dia sadari adalah kejutan
   yang buruk. Tampilkan sebagai tawaran sekali, saat syaratnya terpenuhi:
   *"3 dari 4 karya Anda punya pemberi kerja. Gabungkan ke Pengalaman Kerja?"* — dengan
   **pratinjau sebelum/sesudah** sebelum pengguna memutuskan. Setelah aktif, tiap item dipasangkan
   ke entri pengalaman induknya (`parentPengalamanId`) dengan validasi rentang tanggal.
...
- `verifikator` **tidak dicetak** di CV — ia metadata untuk kualitas penulisan dan referensi
  ...
  Nama orang lain tidak boleh muncul di berkas ekspor mana pun (§5, kewajiban PDP).
- **Bagian kosong tidak boleh mencetak judul.** Bila `aktif: true` tapi nol item, judul bagian
  tidak muncul sama sekali di PDF/Word/TXT.
...
9. **Tidak ada satu pun tempat di UI yang menyebut angka hasil penilaian sebagai "Skor ATS"**,
   dan tooltip penjelas (§3.7) muncul di sebelah kedua angka. Kata "ATS" sebagai istilah kategori
   — nama produk, judul beranda, penjelasan masalah — **tetap boleh dipakai**.
...
19. ⭐ **Rubrik skor §9.2 diterapkan apa adanya** — ada uji unit yang membuktikan Q dan R
    menghasilkan nilai 0–3 sesuai tabel, dan bahwa `rentangItemIdeal` dengan batas atas `null`
    tidak membuat perhitungan P meledak.
20. ⭐ **Pengguna berjenjang `mahasiswa` dapat mencapai skor "Kekuatan Bukti" ≥70** dengan 2 item
    tugas kuliah yang terisi lengkap tanpa verifikator (§9.5). Kalau tidak bisa, §9.5 belum jalan.
21. ⭐ **Bagian portofolio aktif tapi nol item tidak mencetak judul bagian kosong** di PDF, Word,
    maupun TXT.
22. ⭐ **Item dengan `parentPengalamanId` yang tanggalnya di luar rentang entri induk memunculkan
    peringatan**, dan kata kuncinya tidak dihitung dua kali di Kecocokan Lowongan.
23. ⭐ **Data `verifikator` ikut terhapus** saat pengguna menghapus akun, dan tidak muncul di
    berkas ekspor mana pun.
24. **Ambang SKP dibaca dari berkas data** (`ambang-profesi.ts`), bukan konstanta di dalam kode,
    dan sanggahannya tampil di bawah progress bar.
...
Setelah tiap fase: jalankan aplikasi, buktikan hasilnya, laporkan berkas yang berubah,
lalu **berhenti dan tunggu** (§0 aturan 7).

**⭐ Pembagian sesi — jangan kerjakan tujuh fase dalam satu sesi.**

| Sesi | Fase | Kenapa dipisah |
|---|---|---|
| 1 | Fase 1–2 | fondasi. Kalau model data atau migrasi salah, semuanya salah. Uji buka-simpan-ekspor CV lama dulu sebelum lanjut. |
| 2 | Fase 3–4 | paling banyak berkasnya (form + 10 template + 4 eksportir); butuh konteks bersih |
| 3 | Fase 5 | **jangan digabung dengan sesi 2** — mengubah angka sambil mengubah struktur membuat penyebab pergeseran skor tidak bisa dilacak |
| 4 | Fase 6–7 | tambahan di atas fondasi yang sudah terbukti jalan |
...
- **Mengarang rumus skor.** Rubrik §9.2 dipakai apa adanya.
- **Mengaktifkan sakelar "Gabung ke Pengalaman Kerja" secara otomatis** tanpa persetujuan pengguna.
- **Menampilkan nama verifikator** di CV atau berkas ekspor apa pun.
- **Menuliskan ambang SKP sebagai konstanta di dalam kode.**
- **Menghapus kata "ATS" dari nama produk dan copy beranda** — yang diganti hanya nama angkanya (§3.7).
- **Mengerjakan lebih dari satu sesi fase sekaligus** tanpa diminta (§15).
...
| Daftar 26 `SectionType` Textkernel (§3.2, §3.8) | tidak bisa diverifikasi ulang dari dokumentasi publik | **jangan cetak daftar enum itu di UI atau dokumentasi pengguna** — pakai hanya sebagai alasan internal. Kesimpulan desainnya tetap aman karena `gabungKePengalaman` dibuat sebagai pilihan, bukan paksaan. |
