...
| `src/components/ink/SamuraiIntro.tsx` | Intro pembuka; diputar setiap kali halaman dimuat ulang, dan dapat dilewati dengan ketukan, klik, tombol, atau gulir. Siluetnya SVG sebaris, bukan berkas gambar |
...
| `src/lib/intro.ts` | Store "intro perlu diputar atau tidak", pola yang sama dengan `theme.ts`. Sejak sesi 10 penandanya variabel modul, bukan localStorage - lihat kepala berkasnya |
...
| **Penandanya variabel modul, bukan localStorage** | Muat ulang halaman memuat ulang modulnya juga, sehingga intronya kembali; berpindah halaman di dalam aplikasi tidak, sehingga menekan tombol kembali ke beranda tidak memutarnya lagi. Persis dua perilaku yang dibutuhkan, tanpa satu pun pemeriksaan tambahan. |
